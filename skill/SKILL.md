...
description: Find code by lexical/structural search using the file-sql MCP instead of grepping and reading whole files. Use when locating where a feature or bug lives, jumping to a symbol's definition, or orienting on what changed recently in the repo.
---

# file-sql: search the codebase efficiently

This repo is indexed by **file-sql**, an MCP server that does AI-free lexical +
structural code search by default. Prefer its tools over blind `grep`/`rg` and
over reading whole files: they return ranked files with a short summary and the
exact matching line range, so you read ~20 lines instead of 500.

## When to use which tool

- **`search_code`** - your default for "where is X handled?" Use likely code
  terms plus a short description of the behavior/concept. Optional `categories`
  (backend, frontend, devops, test, config, docs, data, build) and `path_prefix`
  narrow the search. Read the returned `path` at the given `start_line`-`end_line`
  before opening the whole file.
